...
fn assert_not_impl_traceable(ty: &syn::Type) -> proc_macro2::TokenStream {
    quote!(
        const _: fn() = || {
            ...
            struct Invalid0;
            // forbids JSTraceable
            impl<T> NoTraceOnJSTraceable<Invalid0> for T where T: ?Sized + mozjs::gc::Traceable {}
            ...
            where
                K: mozjs::gc::Traceable + std::cmp::Eq + std::hash::Hash,
...
fn js_traceable_derive(s: synstructure::Structure) -> proc_macro2::TokenStream {
    ...
    for param in ast.generics.type_params() {
        ...
        where_clause
            .predicates
            .push(parse_quote!(#ident: mozjs::gc::Traceable))
    }
    ...
    let tokens = quote! {
        ...
        unsafe impl #impl_generics mozjs::gc::Traceable for #name #ty_generics #where_clause {
            ...
            unsafe fn trace(&self, tracer: *mut mozjs::jsapi::JSTracer) {
